...
def file_n_copy(src_file_names : list[str], dest_file_name : str) -> None :
    '''
    @input:
        @src_file_names: list of strings of src file path name.
        @dest_file_name: file name where we want to copy 
    ...
    for file in src_file_names :
        file_copy(file, dest_file_name, isFirstTime)
        isFirstTime = False
...
def main(argc : int, argv : list[str]) -> None :
    ...
    '''
    if(argc != 6) :
        print("please give 5 file names where first 3 are source files and other 2 are destination file !!!")
        sys.exit(-1)
    
    file_n_copy(argv[1:4], argv[4])
    file_n_copy(argv[3:0:-1], argv[5])
